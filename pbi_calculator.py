...
class WZ模型计算器:
    ...
    def __init__(self):
        ...
        self.水常数_keV_per_mm = 8.5                # 水的经验常数
        self.射程参数_g_per_cm2_keV__alpha = 6e-6             # 射程参数k，g_per_cm2_per_keV**alpha
    ...
    def 计算径向剂量(self, 半径_nm, Z, 能量_MeV_per_amu, 密度_g_per_cm3, 电离能_eV=12.0):
        ...
        θ = self.射程参数_g_per_cm2_keV__alpha * (电离能_keV ** α)  # 阈值参数
        W = 2 * self.电子质量_keV * β**2 * γ**2  # 最大能量转移
        T = self.射程参数_g_per_cm2_keV__alpha * (W ** α)  # 特征射程
        
        # 步骤3：距离和密度修正
        半径_cm = 半径_nm * 1e-7  # 转换为cm
        t_g_per_cm2 = 半径_cm * 密度_g_per_cm3  # 密度修正的距离
        ...
        # 步骤5：WZ模型主要计算
        水常数_keV_per_cm = self.水常数_keV_per_mm * 1e1  # 转换为keV/cm
...
• 原子密度：分子密度×36"""
    ...
    def 执行计算并绘图(self):
        ...
        try:
            ...
            # 步骤4：应用归一化因子
            归一化剂量 = 剂量数组 * g因子 / 能量分布信息['累积能量'] * 阻止本领

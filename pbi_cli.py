...
class SRIM数据处理器:
    ...
    def __init__(self):
        ...
        self.srim密度_g_per_cm3 = 1.5
        
    def 计算能量损失(self, 初始能量_MeV_per_amu, pbi厚度_um, pbi密度_g_per_cm3=1.5):
        密度修正因子 = pbi密度_g_per_cm3 / self.srim密度_g_per_cm3
        修正后阻止本领_keV_per_um = self.srim阻止本领_keV_per_um * 密度修正因子
        总能量损失_keV = 修正后阻止本领_keV_per_um * pbi厚度_um
...
class WZ模型计算器:
    ...
    def 计算径向能量分布(self, 半径数组_nm, Z, 能量_MeV_per_amu, 密度_g_per_cm3, 电离能_eV=12.0):
        ...
        单分子能量数组_eV = (剂量数组 * 1000.0) / 分子数密度_per_nm3
        单原子能量数组_eV = (剂量数组 * 1000.0) / 原子数密度_per_nm3
        ...
        return {
            ...
            '单原子能量_eV': 单原子能量数组_eV,
            '累积能量': 累积能量,
            '分子数密度_nm3': 分子数密度_per_nm3,
            '原子数密度_nm3': 原子数密度_per_nm3,
            '每分子原子数': 每分子原子数,
